...
def draw_reange_RSCM(x1,y1,distance_h,angle):
    end_x = int(x1 - distance_h * np.cos(np.radians(angle)))
    end_y = int(y1 + distance_h * np.sin(np.radians(angle)))
    return end_x,end_y
def points_RSCM(x1,y1,x2,y2):
    x3    = x2 + (x2-x1)
    y3    = y2 + (y2-y1)
    return x1,y1,x3,y3
def regis_RSCM(ultra_image,mri_image):
    ...
    (x1, y1) = ori[0]
    (x2, y2) = ori[1]
    x1,y1,x2,y2 = points_RSCM(x1,y1,x2,y2)
    ...
    end_x, end_y = draw_reange_RSCM(x1, y1, distance_h, angle)
    end_x_1, end_y_1 = draw_reange_RSCM(x2, y2, distance_h, angle)
    m1 = (x1, y1)
    m2 = (end_x, end_y)
    m3 = (x2, y2)
    m4 = (end_x_1, end_y_1)
...
def diejia_RSCM(image_path,img_1,m1,m2,m3,m4):
    image = cv2.imread(image_path)
    image = cv2.resize(image, (501, 501))
    hh,ww,c = image.shape
    image_RSCM = np.zeros((hh,ww,c))
    # 定义超声图像的尺寸
    area0 = 0
    for i in range(hh):
        for j in range(ww):
            if np.all(image[i][j]==[0,0,128]):  #RSCM
                image_RSCM[i][j] = [255,255,255]
                area0 = area0+1
    h, w, c = img_1.shape
    ...
    cv2.imwrite("./test.png",image_RSCM)
    image_1 = cv2.imread("./test.png")
    re = cv2.add(image_1, result)

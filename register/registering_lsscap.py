...
def draw_reange_LSSCap(x1,y1,distance_h,angle):
    end_x = int(x1 + distance_h * np.cos(np.radians(angle)))
    end_y = int(y1 - distance_h * np.sin(np.radians(angle)))
    return end_x,end_y
def points_LSSCap(x1,y1,x2,y2):
    x3    = x2 - (x1-x2)
    y3    = y2 - (y1-y2)
    return x1,y1,x3,y3
def regis_LSSCap(ultra_image,mri_image):
    ...
    (x1, y1) = ori[1]
    (x2, y2) = ori[0]
    x1,y1,x2,y2 = points_LSSCap(x1,y1,x2,y2)
    x = abs(int(x1) - int(x2))
    y = abs(int(y1) - int(y2))
    ...
    end_x, end_y = draw_reange_LSSCap(x1, y1, distance_h, angle)
    end_x_1, end_y_1 = draw_reange_LSSCap(x2, y2, distance_h, angle)
    # cv2.line(image, (x1, y1), (end_x, end_y), (255, 255, 255), 2)
    # cv2.line(image, (x2, y2), (end_x_1, end_y_1), (255, 255, 255), 2)
    # cv2.line(image, (x1, y1), (x2, y2), (255, 255, 255), 2)
    # cv2.line(image, (end_x, end_y), (end_x_1, end_y_1), (255, 255, 255), 2)
    m1   =  (x1, y1)
    m2   = (end_x, end_y)
    m3   = (x2,y2)
    m4   = (end_x_1, end_y_1)
